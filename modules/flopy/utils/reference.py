...
class SpatialReference(object):
    ...
    def get_grid_line_collection(self, **kwargs):
        ...
        """
        from QSWATMOD2.modules.flopy.plot import ModelMap
    ...
    def export_contours(self, filename, contours,
        ...
        **kwargs : key-word arguments to flopy.export.shapefile_utils.recarray2shp
        ...
        """
        from QSWATMOD2.modules.flopy.utils.geometry import LineString
        from QSWATMOD2.modules.flopy.export.shapefile_utils import recarray2shp
    ...
    def get_2d_vertex_connectivity(self):
        ...
        verts = np.empty((npoints, 2), dtype=np.float64)
        verts[:, 0] = x
        verts[:, 1] = y
        iverts = []
    ...
    def get_3d_shared_vertex_connectivity(self, nlay, botm, ibound=None):
        ...
        verts = np.empty((npoints, 3), dtype=np.float64)
        ...
        if ibound is not None:
            ...
            ivertrenum = np.zeros(npoints, dtype=np.int_)
            for vlist in iverts:
                for iv in vlist:
    ...
    def get_3d_vertex_connectivity(self, nlay, top, bot, ibound=None):
        if ibound is None:
            ...
            ibound = np.ones((nlay, self.nrow, self.ncol), dtype=np.int_)
        else:
            ncells = (ibound != 0).sum()
        npoints = ncells * 8
        verts = np.empty((npoints, 3), dtype=np.float64)
...
def get_spatialreference(epsg, text='esriwkt'):
    ...
    """
    from QSWATMOD2.modules.flopy.utils.flopy_io import get_url_text

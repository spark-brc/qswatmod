...
class ModflowBct(Package):
    ...
    def __init__(self, model, itrnsp=1, ibctcb=0, mcomp=1, ic_ibound_flg=1,
        ...
        self.izod = izod
        self.ifod = ifod
        self.icbund = Util3d(model, (nlay, nrow, ncol), np.float64, icbund,
                             'icbund', )
        self.porosity = Util3d(model, (nlay, nrow, ncol), np.float64,
                               porosity, 'porosity')
        # self.arad = Util2d(model, (1, nja), np.float64,
        #                        arad, 'arad')
        self.dlh = Util3d(model, (nlay, nrow, ncol), np.float64, dlh, 'dlh')
        self.dlv = Util3d(model, (nlay, nrow, ncol), np.float64, dlv, 'dlv')
        self.dth = Util3d(model, (nlay, nrow, ncol), np.float64, dth, 'dth')
        self.dtv = Util3d(model, (nlay, nrow, ncol), np.float64, dth, 'dtv')
        self.sconc = Util3d(model, (nlay, nrow, ncol), np.float64, sconc,
                            'sconc', )
